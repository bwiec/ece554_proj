...
void gpio::xgpio_init()
{
	...
	int status = XGpio_Initialize(&_xgpio, _device_id);
	if (status != XST_SUCCESS)
	{
		cerr << "Initialization of xgpio failed for " << _device_id;
	}
	
	XGpio_SetDataDirection(&_xgpio, 1, 0x00000000); // All outputs
...
void gpio::write(int val)
{
	...
	XGpio_DiscreteWrite(&_xgpio, 1, val);
...
int gpio::read()
{
	...
	int val = XGpio_DiscreteRead(&_xgpio, 1);
